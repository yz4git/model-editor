import {faceGroups,detailedFaceDefaults} from './face-controls.js';
import {deformAnalyticFace} from './face-deform.js';
export const defaults={...detailedFaceDefaults,height:1.72,shoulders:1,waist:1,hips:1,muscle:.35,legs:1,head:1,faceWidth:1,jaw:1,eyes:1,nose:1,lips:1,chestDepth:1,waistDepth:1,hipsDepth:1,noseWidth:1,eyeSize:1,mouthWidth:1,cheekVolume:1,chinLength:1,chinProjection:1,foreheadDepth:1,mouthProjection:1};
const bodyDefinitions=[['体格',[['height','身長',1.45,2.05,.01],['shoulders','肩幅',.8,1.2,.01],['waist','胴まわり',.75,1.35,.01],['hips','骨盤幅',.8,1.25,.01],['chestDepth','胸の厚み',.7,1.4,.01],['waistDepth','腰の厚み',.7,1.4,.01],['hipsDepth','骨盤の厚み',.7,1.4,.01],['muscle','筋肉量',0,1,.01],['legs','脚の長さ',.88,1.12,.01],['head','頭の大きさ',.88,1.12,.01]]]];
export const definitions=[...bodyDefinitions,...faceGroups];
export function sanitize(a={}){a=a&&typeof a==='object'?a:{};const p={...defaults};for(const [,ds] of definitions)for(const [k,,lo,hi] of ds)if(Number.isFinite(a[k]))p[k]=Math.max(lo,Math.min(hi,a[k]));return p;}
...
deformAnalyticFace(verts,normals,p,headBase,scale);
