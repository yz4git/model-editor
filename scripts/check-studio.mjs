...
assert.equal(mesh.rig.length,17);assert.deepEqual(mesh.parts.map(p=>p.name),['Body','Head','Eyes','Hair','Eyelids']);
...
const binary=toGLB(mesh),view=new DataView(binary);assert.equal(view.getUint32(0,true),0x46546c67);assert.equal(view.getUint32(8,true),binary.byteLength);const length=view.getUint32(12,true),json=JSON.parse(new TextDecoder().decode(new Uint8Array(binary,20,length)));assert.equal(json.skins[0].joints.length,17);assert.equal(json.meshes.length,5);assert.deepEqual(json.animations.map(a=>a.name),['Greeting','FaceStudy']);const binLength=view.getUint32(20+length,true);assert.equal(binLength,json.buffers[0].byteLength);for(const b of json.bufferViews){assert.equal(b.byteOffset%4,0);assert(b.byteOffset+b.byteLength<=binLength);}for(const anim of json.animations)for(const c of anim.channels){const s=anim.samplers[c.sampler],input=json.accessors[s.input],output=json.accessors[s.output];assert.equal(output.count,input.count*(c.target.path==='weights'?3:1));}
