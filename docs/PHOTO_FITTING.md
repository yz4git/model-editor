...
Face fitting adjusts `faceWidth`, `jaw`, `eyes`, `lips`. Body fitting adjusts `shoulders`, `waist`, `hips`, `legs`. Optional known height is accepted in the existing 145–205 cm range. Other parameters and the other shape group are preserved. Nose depth, muscle amount and global head size are not identifiable reliably from these frontal measurements and remain unchanged. Open-mouth face detections retain lip thickness by default.
